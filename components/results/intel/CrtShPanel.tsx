  ...
  if (!data) {
    return (
      <div className="space-y-3 py-10 text-center">
        ...
        <p className="font-medium text-muted-foreground">crt.sh aún no tiene resultados</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        {data.subdomains.length > 0 ? (
          <CyberBadge type="info" size="sm" label={`${data.subdomains.length} subdominio(s) descubiertos`} />
        ) : (
          <CyberBadge type="completed" size="sm" label="Sin subdominios en Certificate Transparency" />
        )}
        {data.simulated && (
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-2">
        <CyberCard padding="p-3">
          ...
          <div className="mt-1 font-mono text-[10px] text-muted-foreground">subdominios únicos</div>
        </CyberCard>
        <CyberCard padding="p-3">
          <div className="font-mono text-2xl font-bold leading-none text-foreground">{data.certificate_count}</div>
          <div className="mt-1 font-mono text-[10px] text-muted-foreground">certificados encontrados</div>
        </CyberCard>
      </div>
      ...
      {data.permalink && (
        ...
        >
          Ver búsqueda completa en crt.sh
