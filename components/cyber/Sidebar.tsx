...
function SectionLink({ section, collapsed }: { section: NavSection; collapsed: boolean }) {
  ...
  return (
    ...
    >
      ...
      <Icon className="h-3.5 w-3.5 shrink-0" style={{ color: active ? c.fg : undefined }} />
...
function ExtraLeafLink({
  ...
  const active = useActive(leaf.href)
  ...
  return (
    <Link
      ...
      style={{ ['--c-fg' as string]: c.fg, ['--c-rgb' as string]: c.rgb }}
    >
      <LeafIcon className="h-3 w-3 shrink-0" />
      {!collapsed && <span className="truncate">{label}</span>}
    </Link>
  )
}
  ...
  return (
    <div className="mb-1">
      {!collapsed && (
        <button
          onClick={() => setOpen((o) => !o)}
          className="flex w-full items-center justify-between px-2.5 py-1.5 font-mono text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70 hover:text-muted-foreground"
        >
          <span>{label}</span>
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed left-3 top-3 z-[60] flex h-9 w-9 items-center justify-center rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--card))]/90 backdrop-blur lg:hidden"
      ...
      >
        {/* Brand */}
        <div className={cn('flex h-16 items-center gap-2.5 border-b border-[hsl(var(--border))] px-4', collapsed && 'lg:justify-center lg:px-0')}>
          ...
          <button
            onClick={() => setMobileOpen(false)}
            className="ml-auto flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground hover:text-foreground lg:hidden"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-3">
          <div className="space-y-0.5 px-1.5 pb-2">
            {TOP_LEAVES.map((leaf) => (
              <TopLeafLink key={leaf.id} leaf={leaf} collapsed={collapsed} />
            ))}
          </div>
          ...
          <div className="mt-1 space-y-0.5 px-1.5">
            <Link
              href={DOCS_LEAF.href}
              title={collapsed ? tSidebar('docs') : undefined}
              className={cn(
                'flex items-center gap-2.5 rounded-md px-2.5 py-2 text-xs font-medium transition-all',
            ...
            >
              ...
              {!collapsed && <span>{tSidebar('docs')}</span>}
            </Link>
          </div>
        </nav>
        ...
        <button
          ...
          className="hidden items-center justify-center gap-2 border-t border-[hsl(var(--border))] py-3 text-muted-foreground hover:text-foreground lg:flex"
        >
